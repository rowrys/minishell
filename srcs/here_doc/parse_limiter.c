...
/*   Updated: 2026/02/09 09:07:55 by ykolacze         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_parse_limiter_quote(t_ctx *ctx, char *block, size_t *i,
		char **limiter)
{
	char	*part;
...
static void	ft_parse_limiter_block(t_ctx *ctx, char *block, size_t *i,
		char **limiter)
{
	char	*part;
	size_t	size;
	...
	size = 0;
	while (block[*i + size] && block[*i + size] != '"'
		&& block[*i + size] != '\'')
	{
		size++;
	}
